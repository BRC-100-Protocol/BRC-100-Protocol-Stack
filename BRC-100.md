...
| --------- | ------------------------------------------------------------ | --------- | ----------- |
| p         | Protocol, case insensitive                                   | Yes       | No          |
| op        | Operator, case insensitive                                   | Yes       | Yes         |
| tick      | Ticker: 3,5-100 letters, ":" represents a child application, case insensitive | Yes       | No          |
...
| gtl       | Governance execution time lock, how many hours, default: 24  | No        | Yes         |
| ext       | Extended attribute for extension protocols                   | No        | Yes         |

The following is an example, set the trading tax percentage and trading black hole percentage, and start DAO to open decentralized governance: all tokens need to vote to complete governance, and the governance execution time lock is 48 hours.
...
mint2/mint3 are both mint operators that support computing operations. After users or applications do mint2/mint3, they will get the token, and the state of the application defined in the "from" attribute will be updated. The specific computing logic is defined using the cop (computing operation) attribute. The difference between mint2/mint3 is that mint2 need the application to be the admin of token, and will increase the circulation supply. While mint3 do not need to be admin and will not increase the circulation supply. mint3 will convert the balance of user/application in one application to UTXO, then user/application can use the UTXO on other applications.
...
| ext       | Extended attribute for extension protocols | No        |
 ...
 For example, in the lending application, after user deposited token, application uses "mint2" operator to mint the certification token to the user.
...
```javascript
{
  ...
  "tick": "amm_dex:LP_BRC100_BWBTC",

  "amt": "12",

  "cop": "remove_liquidity",

  "to": "amm_dex:LP_BRC100_BWBTC"
...
r2/r3 are used to recover tokens that were incorrectly burn2ed/burn3ed to applications or users, to mint these tokens to users again by mint2/mint3 operators. Common incorrect types include: token/cop not supported by the application, wrong attributes/parameters, wrong address, etc.

For example, withdraw the eth that was sent to the liquidity pool incorrectly.
...
This chapter will describe the states within the BRC-100 protocol. The indexers should display these states to users to ensure that the states are open and consistent. The states are the results computed by the application according to users' operation and computing operation. States can be variables of application, or the balances of other applications or users in this application, etc. The difference between states and attributes of application is that attributes updating need to be completed through governance, but the states are computed by the public algorithms and rules and do not require governance. The BRC-100 protocol includes the following two states:

• rBalances, Recoverable Balances, represents the balances of tokens that the user can recover in the current application. It represents the tokens burn2ed/burn3ed by users by mistake. Because the application cannot process it by computing logics, it is stored in the application state. Users can use the op: mint2/mint3 and cop: r2/r3 to recover them to their wallet.

• stopped, default: false, indicating whether the application has been stopped. After the application is stopped by governance child application (based on BRC-101 protocol), the state will be updated to true.
